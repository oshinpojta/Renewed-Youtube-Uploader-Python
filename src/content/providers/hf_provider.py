...
class HuggingFaceTextProvider(TextProvider):
    ...
    def generate(self, request: TextGenerationRequest) -> TextGenerationResult:
        base_url = self.config.base_url.rstrip("/") or "https://router.huggingface.co/v1"
        ...
        return TextGenerationResult(
            ...
            model=model,
            text=text,
            notes=["hf_router_chat_completions"],
        )
    ...
    def _extract_text(data: Dict) -> str:
        ...
        if not choices:
            raise ValueError("HuggingFace response had no choices.")
        message = choices[0].get("message", {})
        text = str(message.get("content", "")).strip()
        if not text:
            raise ValueError("HuggingFace response had empty content.")
        return text
